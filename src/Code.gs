 ...
 function sendUserError(message) {
  ...
  function getConfig(request) {
    ...
    config
      .newInfo()
      .setId('instructions')
      .setText('Fill in the form to connect to a JSON data source.');
    ...
    config
      ...
      .setHelpText('Enter the token here, if the URL requires it.')
    ...
    config
      ...
      .setHelpText('Useful with large datasets. Response is cached for 10 minutes.')
  ...
  function fetchData(url, token, cache) {
    ...
    try {
      ...
      sendUserError(
        'Your request cannot be cached. The rows in your dataset are likely to exceed the 100KB cache limit.'
      );
    }
    if (!content) sendUserError('"' + url + '" did not return any content.');
  ...
  function getFields(request, content) {
    ...
    try {
      ...
      sendUserError('The data format of one of your fields cannot be identified.');
    }
    return fields;
  }
